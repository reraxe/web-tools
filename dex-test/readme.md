...
Issues found during the current pilot are tracked in `V1.1_TEST_BACKLOG.md`.
Repository layout
...
MVP workflow
Create an inbound purchase batch for a booster box, packs, purchased singles, a trade, or existing inventory.
Add front/back scans in the browser, or save scanner images into that batch's watched folder.
Select one image pair or a complete scan batch; Dex pairs files and immediately assigns a unique SKU to every physical card.
Review and edit cards from the multi-card batch grid.
Finish the batch and print all queued 2 x 1 labels.
Scan sleeve QR codes into an eBay or TCGplayer outbound order.
