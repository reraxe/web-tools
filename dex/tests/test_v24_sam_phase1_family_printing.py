...
class SamPhase1FamilyPrintingTest(unittest.TestCase):
    ...
    def test_legacy_values_are_not_backfilled_and_migration_is_transactional(self):
        ...
        try:
            ...
            self.app.apply_migrations = lambda db: apply_migrations(db, DEFAULT_MIGRATIONS[:-3])
            self.app.init_db()
            with self.app.connect() as db:
                now = self.app.utcnow()
                batch_id = db.execute(
                ...
                self.assertIsNone(db.execute(
                    ...
                    (DEFAULT_MIGRATIONS[-3].migration_id,),
                ).fetchone())
                self.assertEqual(apply_migrations(db, DEFAULT_MIGRATIONS[-3:-2]), (DEFAULT_MIGRATIONS[-3].migration_id,))
                card = db.execute("SELECT * FROM cards WHERE id=?", (card_id,)).fetchone()
            ...
            source.close()
            failed.execute("DELETE FROM schema_migrations WHERE migration_id=?", (DEFAULT_MIGRATIONS[-3].migration_id,))
            ...
            with self.assertRaises(MigrationError):
                apply_migrations(failed, DEFAULT_MIGRATIONS[-3:-2])
            self.assertIsNone(failed.execute(
                "SELECT 1 FROM schema_migrations WHERE migration_id=?", (DEFAULT_MIGRATIONS[-3].migration_id,)
            ).fetchone())
            self.assertIsNone(failed.execute(
