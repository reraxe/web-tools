    ...
    def test_disposable_v2_copy_migrates_once_without_backfill(self):
        ...
        try:
            self.assertEqual(
                ...
                tuple(migration.migration_id for migration in DEFAULT_MIGRATIONS[:-3]),
            )
            self.assertEqual(apply_migrations(db, DEFAULT_MIGRATIONS[:-3]), ())
            ...
            self.assertEqual(
                ...
                [(migration.migration_id,) for migration in DEFAULT_MIGRATIONS[:-3]],
            )
            index = db.execute(
