...
The v1a audited multi-evidence intake is a frozen assisted-use lane: it suggests a One Piece family, freezes its bounded candidates/evidence first, and only then opens the descriptive local catalog for operator confirmation or correction. The immutable SAM result, operator decision, optional independently verified truth, and forensic delta remain separate append-only records. Operator corrections are never automatic training labels. Only explicit family confirmation/correction may change inventory identity; exact printing remains operator-only.
...
- UPC remains commercial-product identity only. Phase 3 creates no physical sealed-unit identity, processing batch, basis, receipt fact, SAM match, or portfolio result.
- Phase 4 stores multiple private receipt/source artifacts outside SQLite behind a provider-neutral contract. SQLite keeps metadata, SHA-256 integrity state, acquisition linkage, retry status, durable tombstones, and append-only events. Manual entry remains available during provider failures. The local filesystem adapter is active; the Google Drive-compatible boundary is unconfigured and exposes no public links or credentials.
- Phase 5 adds provider-neutral receipt intelligence. The active provider privately parses text-layer PDFs; image OCR and external providers remain unconfigured. SQLite stores normalized candidates, confidence, source location, matching/classification, allocation proposals, and audit provenance—not raw OCR or document content. Clean multi-line receipt evidence uses direct merchandise plus shared components proportional to merchandise value with deterministic remainder cents ordered by immutable acquisition-line ID. Final acquisition confirmation remains the authority gate.
- The pre-phase UX consistency hotfix establishes a standing disclosure rule: operator-controlled open/closed state survives frontend rerenders, while Critical/Needs Attention may force the affected section open. It also adds recoverable draft-acquisition tombstones and reasoned confirmed-acquisition cancellation. No acquisition is hard-deleted, and linked downstream/economic history requires the established correction or reversal workflow.
- Phase 6 projects confirmed product lines into the existing homogeneous batch model only after an explicit backend preview and confirmation. Pack/Sealed lines may be split across Keep Sealed, Rip/Open, and Decide Later; Single Cards may enter Scan & Identify or remain undecided. Exact unit identities and landed basis are preserved, scanner intake is never activated implicitly, and all unresolved quantity remains visible and resumable.
- Phase 7 connects One Piece Scan & Identify records to conservative SAM recognition. Structured OPTCG metadata is cached behind a provider boundary; configured local reference originals stay external and unchanged while hashes/features are indexed incrementally. Controlled validation adds optional local-only lower-right card-number OCR with deterministic crops and strict One Piece identifier normalization. Clean scans use two independently preprocessed primary attempts and exit only after agreement; unreadable or disagreeing evidence escalates through the complete bounded fallback. OCR remains corroborating evidence: visual conflicts, missing references, quality warnings, and same-number variants block automatic authority, while OCR failure falls back to the unchanged visual path. Only strong multi-source `AUTO_MATCHED` results or explicit operator confirmation/correction become trusted identity. Ambiguous and unidentified cards stay reviewable without blocking intake, all suggestion/decision evidence is retained, and SAM never changes economics.
...
## v2.2-test RC3 Operator Trial checkpoint

RC3 is a packaging and verification checkpoint for the accepted v2.2-test stack, not a new production phase. DEX is **Digital Encyclopedia Xchange** and SAM is **Search And Match**. JANA (**Judgment, Analytics & Navigation Assistant**) and STRIX (**Storage, Transfer, Repository & Information Xchange**) remain reserved future system names.

Authoritative operation remains with the established SAM v1 and human-review contracts. Challenger v1, Challenger v2, Geometry Challenger v1, and the TCGplayer Commercial Printing Catalog Bridge v1 remain shadow-only or descriptive research. They do not grant recognition authority, route geometry automatically, or change operator behavior in RC3.
