...
## SAM Multi-Evidence Audited Intake

The accepted frozen build `SAM-MULTI-EVIDENCE-BLIND-TRIAL-v1a-AUDIT-20260824` is available in the SAM screen as an assisted One Piece intake path. DEX records the complete bounded suggestion and evidence before enabling catalog verification or any operator decision. Confirming or correcting a family is the only action that may write family identity. Needs Review, Unidentified, rescan, and unverified corrections write audit history only. Exact printing, rarity, treatment, pricing, WOLFF, JANA, and marketplace actions receive no automatic authority.

The embedded recognizer/config component ledger must match fingerprint `dd899b6f73891252395ae9b8d09b43906ad15c656f0a0c739bd15a916c012493` at runtime. The accepted recognizer, OCR, candidate generation, evidence fusion, confidence interpretation, thresholds, and catalog logic are byte-frozen; DEX invokes them through an isolated worker to avoid module collisions with the established SAM service.
